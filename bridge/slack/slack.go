...
type Bslack struct {
	...
	*bridge.Config
	...
	// rtm is only used by the legacy (non-bot-token) bridge.
	rtm *slack.RTM
...
func New(cfg *bridge.Config) bridge.Bridger {
	...
	if token != "" {
		if appToken == "" || !strings.HasPrefix(appToken, "xapp-") {
			cfg.Log.Fatalf("Account %s: %s is missing or invalid (must start with xapp-), got %q",
				cfg.Account, appTokenConfig, appToken)
		}
	}
...
func (b *Bslack) Connect() error {
	...
	// If we have a token we use the Slack websocket-based Socket Mode for both sending and receiving.
	if token != "" && appToken != "" {
...
func (b *Bslack) sendSocket(msg config.Message) (string, error) {
